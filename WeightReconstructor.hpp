...
namespace MDR {
    ...
    public:
        ...
        void load_weight(){
            ...
            string path = "refactored_weight_data/weight.bin";
        ...
        bool reconstruct(uint8_t target_level, const std::vector<uint8_t>& prev_level_num_bitplanes, bool progressive=true){
            ...
            for(int i=0; i<=current_level; i++){
                if(level_num_bitplanes[i] - prev_level_num_bitplanes[i] > 0){
                    ...
                    /*
                    T level_max_weight = compute_max_abs_value(reinterpret_cast<int*>(buffer_weight), level_elements[i]);
                    int exp_max_weight = 0;
                    frexp(level_max_weight, &exp_max_weight);
                    // */
                    auto level_decoded_data = encoder.progressive_decode(level_components[i], level_elements[i], level_exp, prev_level_num_bitplanes[i], level_num_bitplanes[i] - prev_level_num_bitplanes[i], i, buffer_weight);
