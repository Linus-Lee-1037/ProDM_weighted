        ...
        std::vector<uint8_t *> encode(T_data const * data, int32_t n, int32_t exp, uint8_t num_bitplanes, std::vector<uint32_t>& stream_sizes, int * weights = NULL) const {
            ...
            for(int i=0; i<n - block_size; i+=block_size){
                for(int j=0; j<block_size; j++){
                    ...
                    // cur_data *= std::pow(2.0, current_weight); // std::pow(2.0, *(weights_pos++)); 
                    T_data shifted_data = ldexp(cur_data, num_bitplanes - exp + current_weight);
            ...
            // leftover
            {
                ...
                for(int j=0; j<rest_size; j++){
                    T_data cur_data = *(data_pos++);
                    int current_weight = *(weights_pos++);
                    // cur_data *= std::pow(2.0, current_weight);
                    T_data shifted_data = ldexp(cur_data, num_bitplanes - exp + current_weight);
                    T_fps signed_int_data = (T_fps) shifted_data;
                    int_data_buffer[j] = binary2negabinary(signed_int_data);
                }
        ...
        T_data * progressive_decode(const std::vector<uint8_t const *>& streams, int32_t n, int exp, uint8_t starting_bitplane, uint8_t num_bitplanes, int level, int * weights=NULL) {
            ...
            if(ending_bitplane % 2 == 0){
                for(int i=0; i<n - block_size; i+=block_size){
                    ...
                    for(int j=0; j<block_size; j++){
                        int current_weight = *(weights_pos++);
                        *(data_pos++) = ldexp((T_data) negabinary2binary(int_data_buffer[j]), - ending_bitplane + exp - current_weight); // / std::pow(2.0, current_weight); 
                    }
                }
                // leftover
                {
